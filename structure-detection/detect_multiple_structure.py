...
def main(in_filename, out_filename, random_seed, start_node, detect_cells_etc, max_rows, max_cols, max_regions, max_fail):
    # Copy file
    ...
    node_structure = DetectNodeStructure(node2node, start_node, max_rows=max_rows, max_cols=max_cols, max_regions=max_regions, max_fail=max_fail)
    ...
    if not detect_cells_etc: return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_regions', metavar='REGIONS', type=int, default=50,
                   help='The maximum number of regions to detect')
    parser.add_argument('--max_fail', metavar='FAILS', type=int, default=25,
                   help='The maximum number of failures per region')
    parser.add_argument('--cells-etc',dest='detect_cells_etc',action='store_true')
    parser.add_argument('--no-cells-etc',dest='detect_cells_etc',action='store_false')
    parser.set_defaults(detect_cells_etc=False)

    parser.add_argument('--max_rows', metavar='max_rows', type=int, default=9999999,
                   help='The maximum number of rows to detect')
    parser.add_argument('--max_cols', metavar='max_cols', type=int, default=9999999,
                   help='The maximum number of cols to detect')

    args = parser.parse_args()
    main(args.infile, args.outfile, args.random_seed, args.start_node, args.detect_cells_etc, max_rows=args.max_rows, max_cols=args.max_cols, max_regions=args.max_regions, max_fail=args.max_fail)
